  ...
  return (
    <div className="w-full min-h-full h-auto flex items-center justify-start flex-col animate-appearing">
      ...
      <form className="flex items-center justify-center flex-col py-44" onSubmit={handleDeleteAccount}>
        ...
        {message ? <p className="text-red-700 text-xl xs:text-xl mt-5 text-center">{message}</p> : null}
        <button
          className="px-10 py-5 mt-5 bg-orange-400 disabled:text-opacity-50 text-white shadow-xl lg:hover:bg-orange-300 lg:disabled:hover:bg-orange-400 lg:disabled:hover:bg-opacity-50 disabled:bg-opacity-50 lg:disabled:cursor-not-allowed rounded-md"
          disabled={isDisabled}
        >
          {!isPending ? (
            'DELETE YOUR ACCOUNT'
